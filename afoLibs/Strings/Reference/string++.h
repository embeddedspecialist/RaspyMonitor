...
private:
    ...
    char *sett_Str(char *target, size_t size, const char *src);
    char *addt_Str(char *target, size_t size, const char *src);
    char *newt_Str(const char *src, size_t size = 0);
    void delt_Str(char *str);
    ...
    inline char *dupt_Str(const char *src, size_t size = 0)
    {return newt_Str(src, size);};
...
public:
	 ...
	 */
	t_Str(const char *str);

	/**
	 * Create a t_Str from std::string.
	 *
	 * @param string from std::string to copy from.
	 */
	t_Str(std::string string);
	 ...
	 */
	t_Str(const t_Str &str, size_t offset, size_t len = npos);
	 ...
	 */
	t_Str(size_t size, const char *format, ...);
#else
        /**
         ...
         */
        t_Str(size_t count, const char *str);
#endif
	 ...
	 */
	t_Str(size_t count, const char fill = ' ');
	 ...
	 * @param offset offset to start from.
	 */
	unsigned count(const t_Str &s, size_t offset = 0) const;
	 ...
	 */
	size_t find(const t_Str &s, size_t offset = 0, unsigned instance = 1) const;
	 ...
	 * @param offset offset to start from.
	 */
	size_t rfind(const t_Str &s, size_t offset = 0) const;
	 ...
	 */
	void insert(size_t start, const t_Str &str);
	...
	void replace(size_t start, size_t len, const char *text, size_t count = 0);
         ...
         */
	void replace(size_t start, size_t len, const t_Str &string);
	...
	inline size_t find(unsigned instance, const char *text, size_t offset = 0, size_t len = 0) const
         ...
         * @param offset offset to start at.
         */
	inline size_t find(unsigned instance, const t_Str &string, size_t offset = 0) const
	 ...
	 */
	inline t_Str substr(size_t start, size_t len) const
		{return t_Str(*this, start, len);};
	 ...
	 * Append operator.
	 */
	inline t_Str &operator+=(const t_Str &str)
		{append(str); return *this;};

	/**
	 * Append operator.
	 */
	inline t_Str &operator+=(char c)
		{add(c); return *this;};

	/**
	 * Append operator.
	 */
	inline t_Str &operator+=(const char *str)
		{append(str); return *this;};

	/**
	 * Append operator.
	 */
	inline t_Str &operator+=(const std::string &str)
	 ...
	 */
//	friend __EXPORT std::istream &getline(std::istream &is, t_Str &str, char delim = '\n', size_t size = 0);

	/**
	 * Stream the content of our string variable directly to a C++
	 * streaming source.
	 */
//	friend __EXPORT std::ostream &operator<<(std::ostream &os, const t_Str &str);

	/**
	 * Stream input into our variable.
	 */
//	inline friend std::istream &operator>>(std::istream &is, t_Str &str)
//		{return getline(is, str);};
	 ...
	 */
// 	friend __EXPORT int strprintf(t_Str &str, size_t size, const char *format, ...);
#endif

        bool operator<(const t_Str &str) const;
	...
	bool operator>(const char *str) const;
         ...
         * Append operator
         */
        inline t_Str &operator+=(int i)
                {append(16, "%d", i); return *this;};

        inline t_Str &operator+=(unsigned int i)
                {append(16, "%u", i); return *this;};

        inline t_Str &operator+=(long l)
                {append(16, "%l", l); return *this;};

        inline t_Str &operator+=(unsigned long l)
                {append(16, "%ul", l); return *this;};

        inline t_Str &operator+=(float f)
                {append(32, "%f", f); return *this;};

        inline t_Str &operator+=(double d)
                {append(32, "%f", d); return *this;};

        inline t_Str &operator+=(short s)
                {append(8, "%hd", s); return *this;};

        inline t_Str &operator+=(unsigned short s)
	...
	inline t_Str &operator=(const t_Str &original)
...
// class __EXPORT St_Str : public t_Str, protected std::streambuf, public std::ostream
// {
// protected:
//         /**
...
// 
// /**
//  * The t_StrObject class is used to derive subclasses that use the
//  * t_Str managed memory pool for all space allocations by overriding
//  * new and delete operators.  Due to size limits, t_StrObject should
//  * not hold very large objects.
//  *
//  * @author David Sugar <dyfet@ostel.com>
//  * @short Objects managed in reusable t_Str memory pools
//  */
// class __EXPORT t_StrObject
// {
// public:
// 	/**
